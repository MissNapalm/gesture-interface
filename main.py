...
class HandTracker:
    def __init__(self):
        ...
        # Gesture recognition
        self.detected_gestures = {}  # Store detected gestures per hand
    ...
    def detect_ok_sign(self, landmarks):
        """Detect OK sign (thumb and index touching, other fingers extended)"""
        ...
        return (thumb_index_distance < 0.05 and 
    ...
    def detect_open_hand(self, landmarks):
        ...
        thumb_extended = abs(thumb_tip.x - wrist.x) > abs(thumb_mcp.x - wrist.x) * 0.7
    ...
    def detect_peace_sign(self, landmarks):
        ...
        index_extended = self.is_finger_extended(landmarks, 8, 6)
        middle_extended = self.is_finger_extended(landmarks, 12, 10)
        ring_folded = not self.is_finger_extended(landmarks, 16, 14)
        pinky_folded = not self.is_finger_extended(landmarks, 20, 18)
        
        # Check thumb is not prominently extended
        thumb_tip = landmarks[4]
        index_mcp = landmarks[5]
        thumb_close = abs(thumb_tip.y - index_mcp.y) < 0.1
        
        return (index_extended and middle_extended and 
                ring_folded and pinky_folded and thumb_close)
    
    def detect_four_fingers(self, landmarks):
        """Detect four fingers up (no thumb)"""
        thumb_tip = landmarks[4]
        index_mcp = landmarks[5]
        
        # Thumb should be folded/close to hand
        thumb_folded = abs(thumb_tip.x - index_mcp.x) < 0.08
        ...
        return (thumb_folded and index_extended and middle_extended and 
                ring_extended and pinky_extended)
    
    def detect_pointing(self, landmarks):
        """Detect one finger pointing (index extended, others folded)"""
        index_extended = self.is_finger_extended(landmarks, 8, 6)
        middle_folded = not self.is_finger_extended(landmarks, 12, 10)
        ring_folded = not self.is_finger_extended(landmarks, 16, 14)
        pinky_folded = not self.is_finger_extended(landmarks, 20, 18)
        ...
        return (index_extended and middle_folded and ring_folded and 
                pinky_folded and thumb_close)
    
    def detect_three_fingers_up(self, landmarks):
    ...
    def recognize_gesture(self, landmarks):
        """Recognize which gesture is being shown"""
        if self.detect_ok_sign(landmarks):
            return "OK SIGN"
        elif self.detect_four_fingers(landmarks):
            return "FOUR FINGERS"
        elif self.detect_three_fingers_up(landmarks):
            return "THREE FINGERS"
        elif self.detect_peace_sign(landmarks):
            return "PEACE SIGN"
        elif self.detect_pointing(landmarks):
            return "POINTING"
    ...
    def detect_three_finger_gesture(self, landmarks):
        """Detect three finger gesture for wheel control"""
        thumb_tip = landmarks[4]
        thumb_mcp = landmarks[2]
        wrist = landmarks[0]
...
class MainWindow(QMainWindow):
    ...
    def draw_futuristic_text(self, frame, text, position, frame_w, frame_h):
        """Draw futuristic-styled text with glow effect"""
        x, y = position
        ...
        for i in range(3, 0, -1):
            ...
            cv2.putText(frame, text, (x, y), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, thickness, cv2.LINE_AA)
        ...
        cv2.line(frame, (x, y + 5), (x + text_size[0], y + 5), (255, 255, 255), 1)
    ...
    def update_frame(self):
        ...
        self.tracker.detected_gestures = {}
        ...
        if results.multi_hand_landmarks and results.multi_handedness:
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                gesture = self.tracker.recognize_gesture(hand_landmarks.landmark)
                if gesture:
                    self.tracker.detected_gestures[hand_label] = gesture
            ...
            # Show wheel when right hand is on right side
        ...
        # Draw gesture recognition text with futuristic style
        y_offset = 60
        for hand_label, gesture in self.tracker.detected_gestures.items():
            text = f"{hand_label}: {gesture}"
            self.draw_futuristic_text(frame, text, (30, y_offset), frame_w, frame_h)
            y_offset += 60
